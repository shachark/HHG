...
StatsComputer::StatsComputer(TestIO& test_io, ScoreConfigurable& score_params) :
		TestIO(test_io), ScoreConfigurable(score_params)
{
	...
	if (st == UV_GOF_XDP2 || st == UV_GOF_XDP3 || st == UV_KS_XDP2 || st == UV_KS_XDP3 || st == MV_KS_HHG_EXTENDED) {
	...
	if (IS_UV_DF_KS_TEST(st) || st == UV_KS_AD || st == UV_KS_CVM_KS) {
		...
		dintegral_zero_based_idxs = !(st == UV_IND_DDP || st == UV_IND_ADP);
	...
	switch (st) {
		...
		case MV_KS_HHG_EXTENDED:
			...
			switch (uv_score_type) {
				...
				default: error("Unexpected univariate k-sample score specified"); hhg_extended_uvs = NULL;
			}
		break;
...
void StatsComputer::uvs_ks_mds(void) {
	...
	int maxa = min(cc, len / 2); // FIXME a little strange. Is this what we want?
	for (int a = 2; a < maxa; ++a) {
		tmpd = ds_score[clumpnum * cc + a] - lchoose(len - 1, a - 1);
		mlik = max(mlik, tmpd);
	}
#else // sanity check: this should be identical to the original DS (suppose lambda is 1)
	double mlik = ds_score[clumpnum * cc + 1];
	for (int a = 2; a < cc; ++a) {
		tmpd = ds_score[clumpnum * cc + a] - log(len) * (a - 1);
		mlik = max(mlik, tmpd);
	}
#endif
